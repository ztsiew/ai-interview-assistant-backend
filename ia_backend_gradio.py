...
class InterviewSession:
    def __init__(self):
        ...
        self.analysis_followup = "Waiting for data..."
        self.analysis_transition = "Waiting for data..."
...
def transcribe(audio):
    ...
    # Windows Fix: Create, Close, then Use
    ...
    def callback(indata, frames, time, status):
        ...
        if text.strip():
            ...
            session.analysis_followup = analyze_chunk(
                f"Suggest 2 follow-ups based on plan: {plan_str}", recent, session.custom_prompt
            )
            session.analysis_transition = analyze_chunk(
                f"Identify current topic and suggest transition for plan: {plan_str}", recent, session.custom_prompt
            )
            session.analysis_empathy = analyze_chunk(
                "If emotional/stuck, provide empathy. Else 'Status: Normal'", recent, "Be supportive."
            )
